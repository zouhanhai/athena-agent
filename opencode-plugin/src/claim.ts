 ...
 * `GitClaimLock` (git-lock.ts); it does NOT reimplement the git-lock logic. The
 * claim is ONE commit: the ticket md (status/assignee/session_id + claim row in
 * the Progress Log).
 *
 ...
 * to the Progress Log, sets status/assignee/session_id, and commits ticket +
 * claim row in ONE commit, then pushes. On a lost race the local board is
 * resynced and a `ClaimConflictError` is thrown.
 */
