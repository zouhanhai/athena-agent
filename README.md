...
Employee browser → Tailscale → 6900XT Portal (Vue frontend)
  ...
  └─ 🎫 Kanban         → Portal backend → Pi-driven task flow (TS)
...
1. **M1**: Project skeleton + AgentSession personal conversation
   ...
   - Corresponds to: G1 (Project skeleton + AgentSession)

2. **M2**: Knowledge Graph (LightRAG) + Wiki (llm_wiki)
   - Acceptance: LightRAG starts with DeepSeek+Postgres; llm_wiki runs headless serving :19828; Pi can retrieve from both systems via MCP; graph panel iframe displays
   - Corresponds to: G2 (LightRAG + llm_wiki)

3. **M3**: Pi-driven Kanban + Team Conversation
   - Acceptance: git-driven kanban works (G creation / claim lock / PR / Reject); 3 employees can claim tickets in parallel; team conversation shares Pi; pi-intercom coordinates
   - Corresponds to: G3 (Pi Kanban + Team Conversation)
