...
describe("KanbanTab (local GST board, G4.S6.T1)", () => {
  beforeEach(() => {
  ...
  it("shows ONLY the local GST board — the GitHub Project view is a separate tab (G4.S6.T1)", async () => {
    localStorage.setItem("athena.session_token", "tok_1");
    const wrapper = await mountKanbanTab(REPO);

    expect(wrapper.find(".kanban-tree").exists()).toBe(true);
    expect(wrapper.find(".kanban-columns").exists()).toBe(true);
    // No view toggle, no project selector, no GitHub project board here.
    expect(wrapper.find(".kanban-view-toggle").exists()).toBe(false);
    expect(wrapper.find(".kanban-project-board").exists()).toBe(false);
    expect(wrapper.find(".kanban-project-select").exists()).toBe(false);
    wrapper.unmount();
  });
});
