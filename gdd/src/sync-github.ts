 ...
 *
 * GDD runs standalone on the USER's machine (no athena server/DB needed), so
 * the credential is LOCAL-token-first (gdd/src/credential.ts): an explicit
 * --token → `gh auth token` (gh CLI) → `GITHUB_TOKEN` env → the athena employee
 * store ONLY as an optional fallback when running inside athena (DATABASE_URL).
 *
 * Usage:
...
import { scanBoard, defaultBoardRoot, type KanbanBoard } from "./kanban/scan.js";
import {
  buildIssueForSpec,
  buildIssueForTicket,
  createSpecIssue,
...
import {
  ...
  readFeedbackContext,
  writeSyncState,
} from "./kanban/github-feedback.js";
import type { PlanInput } from "./kanban/planning.js";
import { GithubClient } from "./github/client.js";
import type { GithubCredential, GithubProject } from "./github/types.js";
import { readTicketFile, writeBoardFile } from "./kanban/board.js";
import { TICKET_STATUSES, type TicketStatus } from "./kanban/schema.js";
import { athenaEmployeeReader } from "./athena-employee.js";
import { resolveGithubCredential } from "./credential.js";
...
function fail(message: string): never {
  ...
  console.error("usage: sync-github <create|sync|status|pull|feedback|list> [args] [--owner O --repo R --project P --token T --board-root PATH]");
  process.exit(1);
}
...
/** Resolve the GitHub credential: --token → gh auth token → GITHUB_TOKEN → optional athena store. */
async function resolveCredential(): Promise<GithubCredential> {
  const employeeReader = await athenaEmployeeReader();
  return resolveGithubCredential({ token: flag("token"), employeeReader });
}
...
async function requireSpecIssue(github: GithubClient, credential: GithubCredential, specRef: string) {
